...
class MetricsInterceptorTest {
  ...
  @Autowired private MetricsInterceptor interceptor;
  ...
  void shouldRecordServerErrorMetrics() throws Exception {
    ...
    var timer = meterRegistry.find("http.server.requests").tag("outcome", "SERVER_ERROR").timer();

    assertThat(timer).isNotNull();
    assertThat(timer.count()).isEqualTo(1);

    var counter = meterRegistry.find("http.server.errors").tag("status", "500").counter();
